// Created by web3d3v on 24/04/2022.
...
class Bip39 {
    ...
    convenience init(
        _ entropySize: EntropySize = .entropy128,
        salt: String = "",
        wordList: WordList = .english
    ) throws {
        let entropy = try Data.secRandom(entropySize.rawValue / 8)
        try self.init(entropy, salt: salt, wordList: wordList)
    }
    ...
    init(mnemonic: [String], salt: String = "", wordList: WordList = .english) {
        self.mnemonic = mnemonic
        self.salt = salt
        self.wordList = wordList
    }
    ...
    func seed() throws -> Data {
        guard let seed = pbkdf2(
            ...
            salt: ("mnemonic" + salt).data(using: .utf8)!,
            length: 64,
            rounds: 2048,
            algorithm: .hmacSHA512
        ) else {
            throw Bip39Error.failedToDeriveKeyFromMnemonic
        }
...
private extension Bip39 {
    ...
    static func entropyWithCheckString(_ entropy: Data) -> String {
        ...
        let checksumBitLength = uint(entropy.count / 4)
        let checksum = Data(SHA256.hash(data: entropy))
        let checksumStr = binaryString(checksum)
        var entropyStr = binaryString(entropy)

        for i in 0..<checksumBitLength {
            let char = checksumStr[Int(i)]
            entropyStr.append(char)
        }

        return entropyStr
    }

    static func binaryString(_ data: Data) -> String {
        return data.reduce(
            "",
            {
                var str = String($1, radix:2)
