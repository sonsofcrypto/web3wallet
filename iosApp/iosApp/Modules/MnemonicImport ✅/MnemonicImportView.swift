...
final class MnemonicImportViewController: BaseViewController {
    ...
    @IBAction func ctaAction(_ sender: Any) {
        presenter.handle(event: MnemonicImportPresenterEvent.DidSelectCta())
    }

    @IBAction func dismissAction(_ sender: Any?) {
        presenter.handle(event: MnemonicImportPresenterEvent.DidSelectDismiss())
    }
}
...
extension MnemonicImportViewController: UICollectionViewDelegate {
    ...
    func onMnemonicChanged() -> MnemonicImportCell.OnMnemonicChanged {
        {
            ...
            self.presenter.handle(
                event: MnemonicImportPresenterEvent.MnemonicChanged(
                    to: info.mnemonic,
    ...
    func nameDidChange(_ name: String) {
        presenter.handle(event: MnemonicImportPresenterEvent.DidChangeName(name: name))
    }

    func iCloudBackupDidChange(_ onOff: Bool) {
        presenter.handle(event: MnemonicImportPresenterEvent.DidChangeICouldBackup(onOff: onOff))
    }

    func saltSwitchDidChange(_ onOff: Bool) {
        presenter.handle(event: MnemonicImportPresenterEvent.SaltSwitchDidChange(onOff: onOff))
    }

    func saltTextDidChange(_ text: String) {
        presenter.handle(event: MnemonicImportPresenterEvent.DidChangeSalt(salt: text))
    }

    func saltLearnMoreAction() {
        presenter.handle(event: MnemonicImportPresenterEvent.SaltLearnMoreAction())
    }

    func passTypeDidChange(_ idx: Int) {
        presenter.handle(event: MnemonicImportPresenterEvent.PassTypeDidChange(idx: idx.int32))
    }

    func passwordDidChange(_ text: String) {
        presenter.handle(event: MnemonicImportPresenterEvent.PasswordDidChange(text: text))
    }

    func allowFaceIdDidChange(_ onOff: Bool) {
        presenter.handle(event: MnemonicImportPresenterEvent.AllowFaceIdDidChange(onOff: onOff))
    }
}
